...
MYSQL *connect2db();
void clientStationHandler(int clientSockId);
...
int main(int argc, char const *argv[]) {
    ...
    char *stations_query ="SELECT station_id, Latitude ,Longitude  FROM stations";
         ...
         if(listen_status != -1){
             ...
             while(1){
                 ...
                 while( (clientSockId = accept(socketId,(struct sockadrr *) &clientAddr,&clientLen)) )
                 {
                     ...
                     if(pthread_create( &new_thread, NULL,  clientStationHandler, new_sock) < 0)
                     {
...
void clientStationHandler(int clientSockId){
  ...
  char temp_humid_template[]="INSERT INTO observationslip(Date,Station,TIME,Dry_Bulb, Wet_Bulb,SubmittedBy) VALUES('%s','%d','%s','%f','%f','AWS')";
  ...
  if(clientSockId != -1){
      ...
      if(strstr(buffer,stations[counter]->config_2m->txt_2m_value)!=NULL){
        printf("The string is two meter type\n");
       ...
       sprintf(status_query,status_query_template,two_meter_report[V_MCU_2M_KEY],
         ...
         two_meter_report[E64_2M_KEY],stations[counter]->id);
       ...
       if (mysql_query(connection,temp_humid_query)) {
        ...
        printf("The string is 10m meter type\n");
          //printf("%s",buffer);
        ...
        ten_meter_report[DRP_10M_KEY],ten_meter_report[DATE_10M_KEY],ten_meter_report[TIME_10M_KEY],
          ten_meter_report[TXT_10M_KEY],ten_meter_report[E64_10M_KEY],stations[counter]->id);
        if (mysql_query(connection,status_query)) {
            //printf( "%s\n", mysql_error(connection));
         }
        ...
        printf("The string is ground node type\n");
        ...
        sprintf(status_query,status_query_template,
          ...
          ground_node_report[E64_GND_KEY],stations[counter]->id);
        if (mysql_query(connection,status_query)) {
           // printf( "%s\n", mysql_error(connection));
         }
        ...
        printf("The string is Sink node type\n");
        ...
        sprintf(status_query,status_query_template,sink_node_report[V_MCU_SINK_KEY],
          ...
          sink_node_report[TXT_SINK_KEY],sink_node_report[E64_SINK_KEY],stations[counter]->id);
        if (mysql_query(connection,status_query)) {
           // printf( "%s\n", mysql_error(connection));
         }
